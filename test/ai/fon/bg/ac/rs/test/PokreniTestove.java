...
import zadatak1.KucniAparatTest;
...
public class PokreniTestove {
	...
	public static void main(String[] args) {
		pokreniTestoveZaKlasu(KucniAparatTest.class);
		pokreniTestoveZaKlasu(TelevizorTest.class);
		pokreniTestoveZaKlasu(RadioTest.class);
	}
	...
	private static void pokreniTestoveZaKlasu(Class c) {
		...
		String className = c.getName().substring(0, c.getName().length() - 4); // remove last 4 letters "Test"
		
		if (r.wasSuccessful()) {
			System.out.println("------------------------------------------------");
			System.out.format("TEST KLASA: %s %n%n", className);
			System.out.format("Pokrenuto: %-5d Uspešno: %-5d Neuspešno: %-5d %n%n", totalCount, successfulCount, failureCount);
			...
			System.err.format("Pokrenuto: %-5d Uspešno: %-5d Neuspešno: %-5d %n%n", totalCount, successfulCount, failureCount);
			
			// print the test summary only if there are no missing classes, attributes or methods.
			if (thereAreNoLinkageErrors(r.getFailures())) {
				
				// create a map where TestTypes are keys. Values are maps with the method name (from the tested class) is the key, and the value 
				// is a two-dimensional array with the method stats (the number of successful and the number of failed tests for the method).
				...
				if (!testTypeMethodMap.isEmpty()) {
					for (TestTypes testType : testTypeMethodMap.keySet()) {
						...
						for (String methodName : methodsMap.keySet()) {
							...
							System.err.printf("\t %-20s %s/%s %s%n", methodName, methodStats[0], methodStats[0]+methodStats[1], methodStats[1] == 0 ? "OK" : "Neuspesno" );
						}
						System.err.println();
					}
				}
			...
			for (Failure failure : r.getFailures()) {
				...
				if (ex instanceof NoClassDefFoundError) {
					System.err.format("%s. U projektu ne postoji klasa %s %n \t (proverite naziv klase i naziv paketa) %n%n", i++, className);
				} else if (ex instanceof NoSuchFieldError) {
					System.err.format("%s. U projektu ne postoji atribut %s %n\t (proverite tip i naziv atributa) %n%n", i++, ex.getMessage());
				} else if (ex instanceof NoSuchMethodError) {
					String methodName = failure.getDescription().getMethodName().split("_")[1];
					
					System.err.format("%s. U klasi %s ne postoji metoda %s %n\t (proverite naziv metode, povratni tip, tipove i redosled parametara) %n%n", i++, className, methodName);
				} else {
					...
					TestTypes testType = Arrays.stream(TestTypes.values())
							.filter(tt -> tt.namePrefix.equals(methodNameElements[0]))
							.findAny()
							.get();
					
					System.err.format("%s. Test %sza %s %s: %n \t%s %n%n", i++, testVariantName != null ? testVariantName+" " : "", testType.vocative, methodName, failure.getMessage());
				}
			}
	...
	private static boolean isTestFailed(List<Failure> failureList, String testMethodName) {
		return failureList.stream().anyMatch(f -> f.getDescription().getMethodName().equals(testMethodName));
	}
